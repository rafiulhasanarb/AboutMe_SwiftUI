//
//  SeismometerMotionDetector.swift
//  AboutMe_SwiftUI
//
...
class SeismometerMotionDetector: ObservableObject {
    ...
    func updateMotionData() {
        if let data = motionManager.deviceMotion {
            (roll, pitch) = currentOrientation.seismometerAdjustedRollAndPitch(data.attitude)
...
extension SeismometerMotionDetector {
    func started() -> SeismometerMotionDetector {
        start()
        return self
    }
}
...
extension UIDeviceOrientation {
    func seismometerAdjustedRollAndPitch(_ attitude: CMAttitude) -> (roll: Double, pitch: Double) {
        switch self {
